...
from src.transformations.transformation_SSA_1_Prep import Transformation_SSA_Prep
from src.transformations.transformation_SSA_2_Calc import Transformation_SSA
from src.transformations.transformation_SSA_3_Rename import Transformation_SSA_Rename
from src.transformations.transformation_SSA_4_register_allocation import Register_Allocation
...
def test():
    ...
    Optimizer(Parser('examples/available_expr.c').parse(), [
        Transformation_1_1(),  Transformation_1_2(), Transformation_0()], debug=False).optimize()

    Optimizer(Parser('examples/dead_variables.c').parse(), [
        Transformation_2(), Transformation_0()], debug=False).optimize()
    ...
    Optimizer(Parser('examples/superflous.c').parse(), [
        Transformation_1_1(), Transformation_3(), Transformation_2(), Transformation_0()], debug=False).optimize()

    Optimizer(Parser('examples/decr.c').parse(), [
        Transformation_1_1(), Transformation_1_2(), Transformation_3(), Transformation_2(), Transformation_0()], debug=False).optimize()

    Optimizer(Parser('examples/constant_propagation.c').parse(), [
        Transformation_4(), Transformation_0()], debug=False).optimize()

    Optimizer(Parser('examples/interval_analysis.c').parse(), [
        Transformation_5_0(widen=False), Transformation_0()], widen_strategy="loop_separator", max_narrow_iterations=5, debug=True).optimize()
    ...
    Optimizer(Parser('examples/very_busy.c').parse(), [
        Transformation_5_1(), Transformation_5_2(), Transformation_0(force=True)], debug=False).optimize()
    ...
    Optimizer(Parser('examples/big_program.c').parse(),
              fullPipeline, debug=False).optimize()

    Optimizer(Parser('examples/factorial.c').parse(),
              fullPipeline, debug=False).optimize()

    Optimizer(Parser('examples/inline.c').parse(),
              [Transformation_9()], debug=False).optimize()

    Optimizer(Parser('examples/tail_recursive.c').parse(),
              [Transformation_11()], debug=False).optimize()

    Optimizer(Parser('examples/static_single_assigment_form.c').parse(),
              [Transformation_SSA_Prep(), Transformation_SSA(), Transformation_SSA_Rename(), Register_Allocation()], debug=False).optimize()
